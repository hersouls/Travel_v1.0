...
export function useDeleteTravel(): UseDeleteTravelReturn {
  ...
  const deleteTravel = async (travelId: string): Promise<void> => {
    try {
      ...
      // 2. day_plans 삭제
      const { error: dayPlansDeleteError } =
        travelDayIds.length > 0
          ? await supabase
              .from('day_plans')
              .delete()
      ...
      if (dayPlansDeleteError)
        console.warn('일별 계획 삭제 경고:', dayPlansDeleteError);
      if (travelDaysDeleteError)
        console.warn('여행 일정 삭제 경고:', travelDaysDeleteError);
      if (paymentDeleteError)
        console.warn('결제 이력 삭제 경고:', paymentDeleteError);
      if (collaboratorsDeleteError)
        console.warn('협업자 삭제 경고:', collaboratorsDeleteError);
    } catch (err) {
