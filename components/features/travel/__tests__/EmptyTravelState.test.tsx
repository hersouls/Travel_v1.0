...
describe('EmptyTravelState', () => {
  it('기본 텍스트가 올바르게 표시되어야 함', () => {
    ...
    expect(screen.getByText(/Moonwave Travel과 함께 특별한 여행을 계획하고/)).toBeInTheDocument();
  ...
  it('액션 버튼들이 올바르게 표시되어야 함', () => {
    ...
    expect(screen.getByRole('link', { name: /예시 여행 보기/ })).toBeInTheDocument();
  });
  ...
  it('모든 특징 항목들이 표시되어야 함', () => {
    ...
    expect(screen.getByText('친구들과 함께 여행을 계획하세요')).toBeInTheDocument();
    
    expect(screen.getByText('지도 연동')).toBeInTheDocument();
    expect(screen.getByText('Google Maps로 경로를 확인하세요')).toBeInTheDocument();
    
    expect(screen.getByText('예산 관리')).toBeInTheDocument();
    expect(screen.getByText('여행 예산을 체계적으로 관리하세요')).toBeInTheDocument();
    
    expect(screen.getByText('일정 공유')).toBeInTheDocument();
    expect(screen.getByText('여행 일정을 간편하게 공유하세요')).toBeInTheDocument();
  });
  ...
  it('아이콘들이 올바르게 표시되어야 함', () => {
    ...
    const iconContainer = screen.getByText('첫 번째 여행을 계획해보세요').closest('div')?.querySelector('.bg-gradient-to-br');
  ...
  it('반응형 클래스가 적용되어야 함', () => {
    ...
    const buttonContainer = screen.getByText('새 여행 만들기').closest('div')?.parentElement;
    expect(buttonContainer).toHaveClass('sm:flex-row');
    
    // features 섹션의 grid 클래스 확인 - 직접 선택자 사용
    const featuresGrid = container.querySelector('.grid.grid-cols-1.gap-4.text-sm.sm\\:grid-cols-2');
  ...
  it('Moonwave 브랜드 색상이 적용되어야 함', () => {
    ...
    const primaryColorElements = container.querySelectorAll('.text-moonwave-primary');
    expect(primaryColorElements.length).toBeGreaterThan(0);
    
    // moonwave-secondary 색상이 사용되는지 확인
    const secondaryColorElements = container.querySelectorAll('.text-moonwave-secondary');
